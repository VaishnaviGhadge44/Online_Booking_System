...
const Expired = (props) => {
  ...
  console.log("Expired userData ", userData);
  ...
  const getUsers = async () => {
    ...
    console.log("Hello From Expired Bookings", response.data);
    setBookings(
      response.data.filter((item) => item.userBookingStatus === "EXPIRED")
    );
  };
  const handleCancel = () => {
    setOpen(false);
  ...
  const downloadPdf = async (data) => {
    ...
    if (data.price && data.userBookingStatus == "EXPIRED") {
      doc.setFont("Helvetica", "bold");
    ...
    <div className="site-card-wrapper">
      <div className="row">
        <div className="col-md-12 mt-3 centerHeader">
          <h1 style={{ color: "#113344" }}>Expired Bookings</h1>
        </div>
      </div>
      <Row gutter={16}>
        {bookings.length > 0 ? (
                ...
                <Col span={12}>
                  <Form.Item label="Price">
                    <Input
                      ...
                      placeholder="Price"
                      id="input"
                      disabled={data.userBookingStatus == "EXPIRED"}
