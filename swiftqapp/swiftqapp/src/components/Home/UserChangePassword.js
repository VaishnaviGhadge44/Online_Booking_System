...
import { getUserDetails, updateUserPassword } from "../../service/api";
...
function UserChangePassword() {
  ...
  // Sucess alert after update password
  const key = "updatable";
  ...
  const userDetails = async () => {
    ...
    console.log("old pass response", response.data.userPassword);
    // setUserId(response.data[0].id);
    setUserDetail(response.data);
    setOldPassword(response.data.userPassword);
  };
